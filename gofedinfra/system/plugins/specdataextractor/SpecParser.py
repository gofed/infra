...
class SpecParser:
    ...
    def getDevelSubpackage(self):
        ...
        if devel == "":
            logging.warning("No devel subpackage found")
            return None
        ...
        if devel_main != "":
            if devel_main not in subpkg_keys:
                logging.warning("Devel package speficied by %{devel_main} macro not found")
                return None
            return self.subpackages[devel_main]

        if devel_counter == 2:
            if ("%s-devel" % self.pkg_name) not in subpkg_keys:
                logging.warning("No %s-devel found" % self.pkg_name)
                return None
    ...
    def parse(self):
        ...
        if plines == []:
            raise ValueError("No evaluated lines")
        ...
        if 'main' not in sections:
            raise ValueError("spec file has no main section")

        self.parseMainSection(sections['main'])

        # parse package sections
        if 'package' not in sections:
            raise ValueError("spec file does not define any package section")
        ...
        if 'changelog' not in sections:
            raise ValueError("spec file does not define changelog section")

        self.changelogs = self.parseChangelog(sections['changelog'])
        if self.changelogs == []:
            raise ValueError("no changelog available")
    ...
    def parseMainSection(self, section):
        for line in section:
            ...
            if tag == ():
                logging.warning("can not parse %s" % line)
                continue

            key, value = tag
            if key == "":
                logging.warning("missing tag name for %s" % line)
                continue
