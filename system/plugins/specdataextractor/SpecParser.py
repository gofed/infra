...
class SpecParser:
	...
	def parse(self):
		...
		if 'main' not in sections:
			logging.error("spec file has no main section")
			return False
		...
		if 'package' not in sections:
			logging.error("spec file does not define any package section")
			return False

		self.subpackages = self.parsePackageSections(sections['package'])
		if self.subpackages == {}:
			logging.error("no package section found")
			return False

		# parse changelog
		if 'changelog' not in sections:
			logging.error("spec file does not define changelog section")
			return False

		self.changelogs = self.parseChangelog(sections['changelog'])
		if self.changelogs == []:
			logging.error("no changelog available")
			return False
